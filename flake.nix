{
  ...
  outputs = { self, nixpkgs, soundtrack, poster, video, platformPoster, platformSoundtrack, platformVideo }:
    let
      ...
      perSystem = system:
        let
          pkgs = import nixpkgs {
            inherit system;
            # Google Chrome stands in for Chromium on macOS (see `chromium` below).
            config.allowUnfreePredicate = p: nixpkgs.lib.getName p == "google-chrome";
          };

          # tools/*.mjs spawn `chromium`. nixpkgs has no Chromium for macOS, so
          # there it is Google Chrome of the same major version under that name.
          chromium =
            if pkgs.stdenv.hostPlatform.isDarwin
            then pkgs.writeShellScriptBin "chromium" ''exec ${pkgs.google-chrome}/bin/google-chrome-stable "$@"''
            else pkgs.chromium;
          ...
          render = pkgs.writeShellApplication {
            ...
            runtimeInputs = [ pkgs.nodejs chromium pkgs.ffmpeg ];
            runtimeEnv = { LIT_FONTS = "${fonts}"; LIT_AUDIO = "${soundtrack}"; };
            text = ''exec node "''${LIT_REPO:-$PWD}/tools/render.mjs" "$@"'';
          };
        in
        {
          packages = { inherit fonts litHtml litVendor site render soundtrackM4a; default = site; };
          apps.render = { type = "app"; program = "${render}/bin/lit-explainer-render"; };

          devShell = pkgs.mkShell {
            packages = [ python pkgs.ffmpeg pkgs.nodejs chromium pkgs.jq ];
            LIT_FONTS = "${fonts}";
            LIT_HTML = "${litHtml}";
    ...
    in
    {
      packages = forAllSystems (o: o.packages);
      apps = forAllSystems (o: o.apps);
